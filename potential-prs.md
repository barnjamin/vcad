...
1. [Terminal rasterizer visual improvements](#candidate-pr-8-terminal-rasterizer-visual-improvements) — isolated to `termview` rendering.
2. [Kitty/Ghostty pixel overlay stability](#candidate-pr-6-terminal-pixel-protocol-overlay-stability-and-ghostty-fallback), then [Kitty compression](#candidate-pr-7-faster-kitty-payloads-via-zlib-compression) — related but reviewable separately.
3. [Faster movement / render-dirty loop](#candidate-pr-5-faster-keyboard-movement-and-dirty-viewport-rendering) — likely benefits TUI users independent of UX changes.
4. [Parts-list selection UX](#candidate-pr-3-parts-list-keyboard-navigation-and-explicit-multi-select-ux) and [layout/focus fixes](#candidate-pr-4-tui-layout-cleanup-for-toolbarsidebar-and-hidden-chat-focus-trap).
5. [Render/screenshot command](#candidate-pr-2-tui-renderscreenshot-command).
6. [Chat slash commands and clear chat](#candidate-pr-1-tui-chat-slash-commands-and-chat-history-clearing).
7. [OpenRouter/Ollama backend support](#candidate-pr-9-openrouterollamaopenai-compatible-chat-backends) — largest API-facing change; easiest to review after TUI command changes are settled.
